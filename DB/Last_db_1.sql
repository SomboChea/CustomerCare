...
-- ----------------------------
-- View structure for viewInfo
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[viewInfo]') AND type IN ('V'))
	DROP VIEW [dbo].[viewInfo]
GO

CREATE VIEW [dbo].[viewInfo] AS Select 
	m.id mom_id,
	m.created_at created_date,
	m.email email,
	m.expected_date expect_Date,
	m.fb facebook_name,
	m.logger_id inserted_user_ID,
	dbo.getname(m.name_id,3) mom_name,
	m.refer_id source_ID,
	m.status,
	m.tel_1 ,
	m.tel_2 ,
	m.other,
	m.address_id ,
	dbo.getaddress_str(m.address_id) address
	,
	k.id kid_id,
	dbo.getname(k.name_id,4) kid_name,
	k.sex kid_sex,
	k.dob kid_dob,
	k."order"
	,
	src.memo src_memo,
	src.image src_image,
	src.created_at src_created_date,
	src.email src_mail,
	src.logger_id src_inserted_user_ID,
	dbo.getname(src.name_id,3) src_name,
	src.type_id srctype_id,
	srctype.type srctype,
	src.status src_status,
	src.tel_1 src_tel1,
	src.tel_2 src_tel2,
	src.address_id src_address_id,
	dbo.getaddress_str(src.address_id) src_address
...
from tbl_mom m 
	...
	left JOIN tbl_refer src
	on m.refer_id=src.id
	
	inner join tbl_refer_type srctype
	on src.type_id=srctype.id
GO


-- ----------------------------
-- View structure for viewKidsByMom
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[viewKidsByMom]') AND type IN ('V'))
	DROP VIEW [dbo].[viewKidsByMom]
GO

CREATE VIEW [dbo].[viewKidsByMom] AS SELECT 'Please implement it' as msg
GO


-- ----------------------------
-- View structure for viewLocation
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[viewLocation]') AND type IN ('V'))
	DROP VIEW [dbo].[viewLocation]
GO

CREATE VIEW [dbo].[viewLocation] AS select p.name province,d.name district , c.name commune ,p.id pid , d.id did ,c.id cid from 
(select * from locations where master_id=0) p
LEFT JOIN (select * from locations) d 
on p.id=d.master_id
LEFT JOIN (select * from locations) c on c.master_id=d.id
GO


-- ----------------------------
-- View structure for viewMomBySource
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[viewMomBySource]') AND type IN ('V'))
	DROP VIEW [dbo].[viewMomBySource]
GO

CREATE VIEW [dbo].[viewMomBySource] AS SELECT 'Please implement it' as msg
GO
...
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[insertKid]') AND type IN ('P', 'PC', 'RF', 'X'))
	DROP PROCEDURE[dbo].[insertKid]
GO

CREATE PROCEDURE [dbo].[insertKid]
(
@mom_id INT,
@name NVARCHAR(50),
@sex SMALLINT,
...
BEGIN
	...
	SET @count_row = (SELECT count(*) FROM tbl_kid);
	SET @prev_id = (SELECT IDENT_CURRENT('tbl_kid'));
	
	EXEC @name_id = insertName @name,4;
	
	INSERT INTO tbl_kid(name_id, sex, dob, [order], mom_id)
	VALUES(@name_id, @sex, @dob, @ord_kid, @mom_id);
	
	SET @last_id = (SELECT IDENT_CURRENT('tbl_kid'));
...
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[insertProduct]') AND type IN ('P', 'PC', 'RF', 'X'))
...
BEGIN
	...
	SET @count_row = (SELECT count(*) FROM tbl_product);
	SET @prev_id = (SELECT IDENT_CURRENT('tbl_product'));
	
	EXEC @name_id = insertName @name,6;
	
	INSERT INTO tbl_product(name_id, info, [level], owner)
	VALUES(@name_id, @info, @lev, @owner);
	
	SET @last_id = (SELECT IDENT_CURRENT('tbl_product'));
...
-- ----------------------------
-- Procedure structure for insertPregnent
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[insertPregnent]') AND type IN ('P', 'PC', 'RF', 'X'))
	DROP PROCEDURE[dbo].[insertPregnent]
GO

CREATE PROCEDURE [dbo].[insertPregnent]
(
@mom_id INT,
@call_date DATE,
@why NVARCHAR(255),
...
BEGIN
	...
	SET @count_row = (SELECT count(*) FROM tbl_pregnent);
	SET @prev_id = (SELECT IDENT_CURRENT('tbl_pregnent'));
	
	INSERT INTO tbl_pregnent(mom_id, created_at, why, memo, caller_id, call_date)
	VALUES(@mom_id, GETDATE(), @why, @memo, @caller_id, @call_date);
	
	SET @last_id = (SELECT IDENT_CURRENT('tbl_pregnent'));
...
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[insertUser]') AND type IN ('P', 'PC', 'RF', 'X'))
...
BEGIN
	...
	DECLARE @name_id INT;
	EXEC @name_id = insertName @name, 5;
	
	INSERT INTO tbl_user(fullname_id, username, password, role, created_at)
	VALUES(@name_id, @username, @password, @role, GETDATE());
...
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[getNames]') AND type IN ('P', 'PC', 'RF', 'X'))
...
-- ----------------------------
-- Function structure for getaddress_str
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[getaddress_str]') AND type IN ('FN', 'FS', 'FT', 'IF', 'TF'))
	DROP FUNCTION[dbo].[getaddress_str]
GO

CREATE FUNCTION [dbo].[getaddress_str]
( @c_id AS int 
)
RETURNS varchar(max)
AS
BEGIN
	...
	select @n=concat(province,' , ',district,' , ', commune) from viewLocation where cid=@c_id	
	RETURN @n
END
GO
...
-- ----------------------------
-- Procedure structure for Simple_Single_Condition
-- ----------------------------
IF EXISTS (SELECT * FROM sys.all_objects WHERE object_id = OBJECT_ID(N'[dbo].[Simple_Single_Condition]') AND type IN ('P', 'PC', 'RF', 'X'))
	DROP PROCEDURE[dbo].[Simple_Single_Condition]
GO

CREATE PROCEDURE [dbo].[Simple_Single_Condition]
  @tab AS  VARCHAR(50),
  @col AS  VARCHAR(50),
  @val AS VARCHAR(50) 
AS
BEGIN
	...
	set @strsql=concat('SELECT * from ',@tab,' where "',@col,'" like ''',@val,'''; ');
